...
import { getConfig, getSvgFiles } from '../utils'
...
export default class Gen extends Command {
  ...
  async run() {
    ...
    const normalIcons = await getSvgFiles(resolve(CWD, config?.paths?.normal))

    const colorfulIcons = await getSvgFiles(
    ...
    const html = this.buildHTML({ normal, colorful })
    await writeFile(
  ...
  buildHTML(data: { normal; colorful }) {
